...
The Joint Command Topic Based System implements a ros2_control `hardware_interface::SystemInterface` supporting command and state interfaces through the ROS topic communication layer. It publishes commands as `control_msgs/JointCommand` messages, grouped by interface type (position, velocity, effort).
...
For each `write()` call, one `control_msgs/JointCommand` message is published per interface type (position, velocity, effort). Each message contains the joints that expose that command interface.
